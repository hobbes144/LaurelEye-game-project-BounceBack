...
namespace LaurelEye {
    ...
    /// @return Address of the Matrix data.
    ...
    /// @brief * Operator overloader for VectorTemplated multiplication
    /// @param vec VectorTemplated object to be multiplied with.
    /// @return The result of the multiplication.
    VectorTemplated<float, 4> Matrix4::operator*(const VectorTemplated<float, 4>& vec) const {
        float vectorData[4] = {
            vec[0] * data[0][0] + vec[1] * data[1][0] + vec[2] * data[2][0] + vec[3] * data[3][0],
            vec[0] * data[0][1] + vec[1] * data[1][1] + vec[2] * data[2][1] + vec[3] * data[3][1],
            vec[0] * data[0][2] + vec[1] * data[1][2] + vec[2] * data[2][2] + vec[3] * data[3][2],
            vec[0] * data[0][3] + vec[1] * data[1][3] + vec[2] * data[2][3] + vec[3] * data[3][3]};

        return VectorTemplated<float, 4>(vectorData);
    }

    /// @brief [] Operator overload
    /// @param row Row index to be fetched
    /// @return Row data
    ...
    /// @brief Static function to create a Rotation matrix in all 3 axis in
    ///     reverse for inverse calc
    /// @param angleX Angle of rotation in the x axis.
    /// @param angleY Angle of rotation in the y axis.
    /// @param angleZ Angle of rotation in the z axis.
    /// @return The resulting Rotation matrix.
    ...
    /// @return The resulting Viewing matrix.
    ...
    /// @brief Static function to inverse a matrix
    ...
    /// @brief Transform a direction vector by the Matrix
    /// @param direction Vector to be transformed.
    /// @return The tranformed direction.
    ...
    /// @param radians Angle of the rotation.
    ...
    /// @brief Get a Quaternion representation of the Matrix transform
